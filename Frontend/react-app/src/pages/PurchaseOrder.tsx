...
export default function PurchaseOrderPage() {
  ...
  // Sync company data into form when it becomes available
  useEffect(() => {
    if (!company) {
      return;
    }
  ...
  // Don't render the page if not authenticated or wrong role
  // Company users should use their own dashboard, not the PO creation page
  if (!isAuthenticated || user?.role === "ADMIN" || String(user?.role || "").toLowerCase() === "company") {
    return null;
  }
            ...
            <tbody>
              {formData.lineItems.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td className="col-amount">Γé╣ {item.amount.toFixed(2)}</td>
      ...
      <div className="footer-section">
        <div className="totals-group">
          <div className="total-row total-final">
            <span className="total-label">Total: Γé╣ {formData.total.toFixed(2)}</span>
          </div>
        </div>
