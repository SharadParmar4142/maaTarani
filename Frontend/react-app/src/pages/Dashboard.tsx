...
import { PartyPopper, Send, CheckCircle2, Package, Truck } from "lucide-react";
...
export default function UserDashboardPage() {
  ...
  const [activeSection, setActiveSection] = useState<"all" | "needsReceiving" | "completed" | "rejected">("all");
  ...
  useEffect(() => {
    ...
    // Company-role users should never land on the user dashboard
    if (!isLoading && user && String(user.role).toLowerCase() === 'company') {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (activeSection === "completed") {
      return orders.filter((order) => order.status === "FINAL_DELIVERY");
    }
  ...
  const finalizeReceiving = async (orderId: string) => {
    ...
    try {
      ...
      setTruckSummaryByOrder((prev) => ({ ...prev, [orderId]: response.data }));
      setStatusMessage("Receiving sent successfully.");
      setCelebrationOrderId(orderId);
      setSelectedOrder((prev) => (prev && prev.id === orderId ? { ...prev, status: "FINAL_DELIVERY" } : prev));
      showToast("✅ Receiving sent! Your order is now fully completed.", 'success');
      setError("");
      await loadOrders();
    } catch (err: any) {
      setError(err.message || "Failed to finalize receiving");
      showToast(err.message || "Failed to send receiving", 'error');
    } finally {
  ...
  const isCompanyRole = user && String(user.role).toLowerCase() === 'company';
  if (isLoading || !isAuthenticated || isAdmin || isCompanyRole) {
    return null;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8 md:px-8">
      {/* Toast Notification */}
      {toast && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-4 rounded-xl shadow-xl text-sm font-semibold transition-all ${
          toast.type === 'success' ? 'bg-emerald-50 border border-emerald-200 text-emerald-800' : 'bg-red-50 border border-red-200 text-red-800'
        }`}>
          <span>{toast.msg}</span>
          <button onClick={() => setToast(null)} className="ml-2 text-lg leading-none opacity-60 hover:opacity-100">×</button>
        </div>
      )}
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link to="/purchase-order" className="rounded-lg bg-[#c41e3a] px-4 py-2 font-semibold text-white hover:bg-[#a01830]">
              Create Purchase Order
            </Link>
            <Link to="/" className="rounded-lg border border-gray-300 bg-white px-4 py-2 font-semibold text-gray-700 hover:bg-gray-100">
              Back to Home
            </Link>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm font-semibold ${
                activeSection === "completed" ? "bg-emerald-200 text-emerald-900" : "bg-gray-100 text-gray-700"
              }`}
            >
              ✅ Completed ({orders.filter((order) => order.status === "FINAL_DELIVERY").length})
            </button>
            <button
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="border-b bg-gray-50 text-gray-600">
                  <tr>
                    ...
                    <th className="px-3 py-2">PO Code</th>
                ...
                <tbody>
                  {filteredOrders.map((order) => (
                    <tr key={order.id} className="border-b">
                      ...
                      <td className="px-3 py-2">
                        {order.status === 'PENDING' ? (
                          <span
                            ...
                            className="font-mono text-xs bg-amber-50 border border-amber-200 text-amber-800 px-2 py-0.5 rounded cursor-pointer hover:bg-amber-100"
                            onClick={() => { navigator.clipboard.writeText(order.uniqueId || ''); }}
                          >
                            {String(order.uniqueId || '').slice(0, 8)}… 📋
                          </span>
                        ) : (
                      ...
                      <td className="px-3 py-2">₹{Number(order.total || 0).toLocaleString('en-IN')}</td>
                      <td className="px-3 py-2">
        ...
        {selectedOrder && (
          <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
            ...
            {selectedOrder.status === 'PENDING' && selectedOrder.uniqueId && (
              <div className="mb-5 rounded-xl border-2 border-amber-300 bg-amber-50 p-4">
                <p className="text-xs font-bold text-amber-700 uppercase tracking-wide mb-1">🔑 PO Claim Code — Share with your supplier</p>
                <div className="flex items-center gap-3">
                  <code className="flex-1 font-mono text-base font-bold text-amber-900 tracking-widest bg-white border border-amber-200 rounded-lg px-3 py-2">
                    {selectedOrder.uniqueId}
                  </code>
                  <button
                    onClick={() => { navigator.clipboard.writeText(selectedOrder.uniqueId); setStatusMessage('Code copied to clipboard!'); setTimeout(() => setStatusMessage(''), 2000); }}
                    className="px-3 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold transition-all"
                  >
                    Copy
                  </button>
                </div>
                <p className="text-xs text-amber-600 mt-2">Share this code with the company/supplier. They enter it on their dashboard to claim and approve this PO.</p>
              </div>
            )}
            ...
            {/* ─── Completed PO Summary (FINAL_DELIVERY) ─── */}
            {selectedOrder.status === 'FINAL_DELIVERY' && truckSummaryByOrder[selectedOrder.id] && (
              <div className="mb-6 rounded-xl border-2 border-emerald-200 bg-emerald-50 p-5">
                <div className="flex items-center gap-2 mb-4">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                  <h3 className="text-base font-bold text-emerald-800">Order Completed — Receiving Summary</h3>
                </div>

                {/* Delivery Summary Table */}
                {truckSummaryByOrder[selectedOrder.id].remainingByItem.length > 0 && (
                  <div className="overflow-x-auto mb-4">
                    <table className="min-w-full text-sm bg-white rounded-lg overflow-hidden border border-emerald-200">
                      <thead className="bg-emerald-100">
                        <tr>
                          <th className="px-3 py-2 text-left font-semibold text-emerald-800">Item</th>
                          <th className="px-3 py-2 text-right font-semibold text-emerald-800">Ordered</th>
                          <th className="px-3 py-2 text-right font-semibold text-emerald-800">Delivered</th>
                          <th className="px-3 py-2 text-right font-semibold text-emerald-800">Effective Received</th>
                        </tr>
                      </thead>
                      <tbody>
                        {truckSummaryByOrder[selectedOrder.id].remainingByItem.map((item) => (
                          <tr key={item.lineItemId} className="border-t border-emerald-100">
                            <td className="px-3 py-2 text-gray-800">{item.description}</td>
                            <td className="px-3 py-2 text-right text-gray-600">{item.orderedQuantity} MT</td>
                            <td className="px-3 py-2 text-right font-semibold text-emerald-700">{item.deliveredQuantity.toFixed(3)} MT</td>
                            <td className="px-3 py-2 text-right font-bold text-emerald-900">{(item.deliveredQuantity - (truckSummaryByOrder[selectedOrder.id].trucks.filter(t => t.materialLineItemId === item.lineItemId).reduce((s,t) => s + Number(t.userShortageQuantity || 0), 0))).toFixed(3)} MT</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {/* Per-truck Receiving Reports */}
                {truckSummaryByOrder[selectedOrder.id].trucks.filter(t => t.userReceivingUpdatedAt != null).length > 0 && (
                  <div>
                    <h4 className="text-sm font-bold text-emerald-700 mb-2 flex items-center gap-1"><Truck className="w-4 h-4" /> Truck Receiving Reports</h4>
                    <div className="space-y-2">
                      {truckSummaryByOrder[selectedOrder.id].trucks
                        .filter(t => t.userReceivingUpdatedAt != null)
                        .map((truck) => (
                          <div key={truck.id} className="bg-white border border-emerald-200 rounded-xl p-3 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                            <div className="flex-1">
                              <p className="font-bold text-gray-800">{truck.truckNumber}</p>
                              <p className="text-xs text-gray-500 mt-0.5">{truck.materialDescription}</p>
                              <div className="mt-1.5 flex flex-wrap gap-3 text-xs">
                                <span className="text-gray-600">Delivered: <strong>{(truck.effectiveReceivedQuantity + Number(truck.userShortageQuantity || 0)).toFixed(3)} MT</strong></span>
                                ...
                                <span className="text-emerald-700 font-semibold">Net Received: {truck.effectiveReceivedQuantity.toFixed(3)} MT</span>
                              </div>
                              {truck.userReceivingNote && (
                                <p className="text-xs text-gray-500 mt-1 italic">Note: {truck.userReceivingNote}</p>
                              )}
                            </div>
                            <div className="text-right shrink-0">
                              <p className="text-xs text-gray-400">Receiving submitted</p>
                              <p className="text-xs font-semibold text-gray-600">{new Date(truck.userReceivingUpdatedAt!).toLocaleString()}</p>
                            </div>
                          </div>
                        ))}
                    </div>
            ...
            <div className="grid gap-2 text-sm text-gray-700 md:grid-cols-2">
              ...
              <p><span className="font-semibold">Sub Total:</span> ₹{Number(selectedOrder.subTotal || 0).toLocaleString('en-IN')}</p>
              <p><span className="font-semibold">Total:</span> ₹{Number(selectedOrder.total || 0).toLocaleString('en-IN')}</p>
